	...
	: Super(ObjectInitializer)
{
	...
	TGAExporter = CreateDefaultSubobject<UTextureExporterTGA>(TEXT("TGAExporter"));
}
...
bool UStaticMeshExporterITP::ExportText(const FExportObjectInnerContext* Context, UObject* Object, const TCHAR* Type, FOutputDevice& Ar, FFeedbackContext* Warn, uint32 PortFlags /*= 0*/)
{
		...
		StaticMesh->GetMaterial(0)->GetTexturesInPropertyChain(MP_BaseColor, textures, nullptr, nullptr))
	{
		if (textures.Num() > 0)
		{
				...
				UExporter::CurrentFilename.FindLastChar('.', endIdx))
			{
				...
				UExporter::ExportToFile(textures[0], TGAExporter, *outputPath, false);
			}
		}
	}
	...
	File->Logf(TEXT("\t\t{ \"type\":\"position\", \"type\":\"float\", \"count\":3 },"));
	File->Logf(TEXT("\t\t{ \"type\":\"normal\", \"type\":\"float\", \"count\":3 },"));
	File->Logf(TEXT("\t\t{ \"type\":\"texcoord\", \"type\":\"float\", \"count\":2 }"));
	File->Logf(TEXT("\t],"));
